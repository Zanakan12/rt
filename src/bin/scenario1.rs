...
fn main() {
    ...
    let image_width = 400; // Efficient for audit
    let image_height = (image_width as f64 / aspect_ratio) as i32;
    let samples_per_pixel = 50;
    let max_depth = 50;

    let mut world = HittableList::new();
    let mat_diffuse = Rc::new(Lambertian::new(Color::new(0.7, 0.3, 0.3)));
    world.add(Box::new(Sphere::new(Point3::new(0.0, 0.0, -1.0), 0.5, mat_diffuse.clone())));

    let lookfrom = Point3::new(0.0, 0.0, 1.0);
    let lookat = Point3::new(0.0, 0.0, -1.0);
    let vup = Vec3::new(0.0, 1.0, 0.0);
    let cam = Camera::new(lookfrom, lookat, vup, 90.0, aspect_ratio);
